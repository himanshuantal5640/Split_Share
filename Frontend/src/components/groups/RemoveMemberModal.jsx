...
const RemoveMemberModal = ({ isOpen, onClose, onConfirm, groupId, member }) => {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      setError(
        ...
        'Failed to remove member. Verify group owner settings.'
      );
    } finally {
      setLoading(false);
    }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative max-w-md w-full p-6 rounded-2xl border border-slate-900 bg-slate-950 shadow-2xl flex flex-col gap-5 z-10 animate-scale-up">
        <div className="flex justify-between items-center">
          <h3 className="text-xl font-bold text-white">Remove Group Member</h3>
          <button
            onClick={onClose}
        ...
        <div className="flex flex-col gap-3">
          ...
          <p className="text-sm text-slate-300 leading-relaxed">
            Are you sure you want to remove <span className="text-indigo-400 font-semibold">{member.name}</span> (ID: #{member.userId}) from this group circle?
          </p>
          
          <p className="text-xs text-slate-500 leading-relaxed font-semibold">
            * This represents a logical soft-remove. The user will be cataloged under "Membership History", and their leave date will be recorded as the current timestamp.
          </p>
        </div>
